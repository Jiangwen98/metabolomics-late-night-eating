...
#### !!!!! this is keep first 1 hour data only !!!!!! ######
...
### Only keep time-points up-to 1 hour ###
qtc_info <- qtc_info[-which(!(qtc_info$Time %in% c(0,15,30,45,60))), ]
### Only keep time-points up-to 1 hour ###
...
for(m in mtbl){
  ...
  colnames(mtbl_sinplotting)<- c('Category', 'Time.in.Minutes', m, "Median_95CIl_lwr", "Median_95CI_upr")
  ...
  mtbl_sinplotting <- within(mtbl_sinplotting, c(Time.in.Minutes <-  factor(Time.in.Minutes, levels = c("0", "15", "30", "45", "60")),
...
##plotting
if(!file.exists("otp/Line_plots_all_TMs_1h")){dir.create("otp/Line_plots_all_TMs_1h")}
for(df in lineplot_list){
  ...
  p <- plot_graph_line2(df, 'Category', 'Time', mtbl_name = mt_name, ymin = "Median_95CIl_lwr", ymax = "Median_95CI_upr")
  ggsave(filename = paste0("otp/Line_plots_all_TMs/Level-time_Lineplots", "_", mt_name, ".pdf")
...
qtc_info <- rbind(qtc_info, a,b,c,d,e,f)
...
p <- plotIndiv(pls.auc, ind.names = F, ellipse = TRUE, legend = TRUE, 
               title = "", 
               X.label = "Comp1 (13%)", Y.label = "Comp2 (9%)",
...
# ggsave(filename = "otp/PLS-DA_AUC.pdf"
...
### LMM.lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^####
write.csv(p.tab, file = paste0("otp/1hlgAUC_[original]pval_auto11", ".csv"), row.names = F)
write.csv(ES.tab, file = paste0("otp/1hlgAUC_EffectSize_auto11", ".csv"), row.names = F)
write.csv(re.tab, file = paste0("otp/1hlgAUC_LMM_Random_effect_auto11", ".csv"), row.names = T)
### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

# ### LMM.iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^######
# write.csv(p.tab, file = paste0("otp/1hiAUC_[original]pval_auto11", ".csv"), row.names = F)
# write.csv(ES.tab, file = paste0("otp/1hiAUC_EffectSize_auto11", ".csv"), row.names = F)
# write.csv(re.tab, file = paste0("otp/1hiAUC_LMM_Random_effect_auto11", ".csv"), row.names = T)
# ### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
for(i in 2:ncol(p.tab)){
  ...
  #### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ####
  pdf(file = paste0("otp/AUCs_pval_hist/1hlgAUC_Histogram_", name, '.pdf'), width = 11, height = 8)
  #### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

  # #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ####
  # pdf(file = paste0("otp/AUCs_pval_hist/1hiAUC_Histogram_", name, '.pdf'), width = 11, height = 8)
  # #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  #### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #####
  pdf(file = paste0("otp/AUCs_pval_adj/1hlgAUC_BH_vs_qvalue", "_", name, ".pdf"), width = 8, height = 8)
  #### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

  # #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ######
  # pdf(file = paste0("otp/AUCs_pval_adj/1hiAUC_BH_vs_qvalue", "_", name, ".pdf"), width = 8, height = 8)
  # #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #####
write.csv(x = BH_pval_df, file = paste0("otp/1hlgAUC_BH_adj_pvals_auto11", ".csv"), row.names = F)
#### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

# #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #####
# write.csv(x = BH_pval_df, file = paste0("otp/1hiAUC_BH_adj_pvals_auto11", ".csv"), row.names = F)
# #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

report.df <- cbind(ES.tab, BH_pval_df[-1])

#### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #####
write.csv(report.df, file = paste0("otp/1hlgAUC_BHpval_ES_auto11", ".csv"), row.names = F)
#### lgAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

# #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #####
# write.csv(report.df, file = paste0("otp/1hiAUC_BHpval_ES_auto11", ".csv"), row.names = F)
# #### iAUC ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
